...
export default function HealthView({ onBack, onArticleClick, user }: HealthViewProps) {
    ...
    try {
      const response = await fetch('/api/health-records', {
        ...
        body: JSON.stringify({
          ...
          date: newRecord.date,
          status: '正常', // Simple logic for now
          unit: newRecord.type === '血压' ? 'mmHg' : newRecord.type === '血糖' ? 'mmol/L' : newRecord.type === '心率' ? 'bpm' : '℃'
        })
      });
  ...
  const stats = [
    { label: '血压', value: records.find(r => r.type === '血压')?.value || '--', unit: 'mmHg', trend: 'normal', icon: Activity, color: 'text-primary' },
    { label: '血糖', value: records.find(r => r.type === '血糖')?.value || '--', unit: 'mmol/L', trend: 'up', icon: HeartPulse, color: 'text-error' },
    { label: '体重', value: records.find(r => r.type === '体重')?.value || '--', unit: 'kg', trend: 'down', icon: Scale, color: 'text-secondary' },
  ];
              ...
              <div className="space-y-6">
                  ...
                  {['血压', '血糖', '心率', '体温'].map(type => (
                    <button 
                      ...
                      className={cn(
                        ...
                        newRecord.type === type ? "bg-primary text-white" : "bg-surface-container-low"
                      )}
                    >
                ...
                <div className="space-y-4">
                  <input 
                    ...
                    className="w-full p-5 rounded-2xl bg-surface-container-low border-none outline-none font-bold text-xl text-center" 
                  />
                  <input 
                    type="datetime-local" 
