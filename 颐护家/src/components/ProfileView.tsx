import { CalendarDays, Wallet, CheckCircle2, ClipboardList, ShieldCheck, MapPin, Headphones, ChevronRight, LogOut, Star, Loader2 } from 'lucide-react';
...
export default function ProfileView({ onNavigate, onLogout, user }: ProfileViewProps) {
  const [counts, setCounts] = useState({ appointments: 0, pending: 0, completed: 0 });
    ...
    >
      ...
      <section className="relative mb-10">
        <div className="bg-[#f4f4ef] rounded-3xl p-8 flex flex-col items-center text-center relative overflow-hidden">
          ...
          <h1 className="text-2xl font-extrabold text-[#1a1c19] tracking-tight mb-2">
            {user.user_metadata?.full_name || user.phone || user.email?.split('@')[0] || '尊贵的颐护家用户'}
          </h1>
          <p className="text-[#3f4940]/60 font-medium">
            {user.phone ? `手机号：${user.phone}` : '专业守护您的健康生活'}
          </p>
        </div>
      </section>
...
function ServiceProgressItem({ icon: Icon, label, color, bg, onClick, count }: any) {
  return (
    <button 
      onClick={onClick}
      className="bg-white p-5 rounded-2xl flex flex-col items-center justify-center gap-3 shadow-[0_4px_20px_rgba(0,0,0,0.02)] active:scale-95 transition-all relative"
    >
      {count > 0 && (
        <span className="absolute top-2 right-2 w-5 h-5 bg-error text-white text-[10px] font-bold rounded-full flex items-center justify-center">
          {count}
        </span>
      )}
